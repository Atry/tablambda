...
# (display name, app, input key, heavy). Heavy cells (the bootstrap) run only under FOL_REGEN_DEFUN=1.
_CELLS = [
...
def _measure_editdistance(approach: str, input_key: str) -> "tuple[float, float, int, str]":
    ...
    if approach == "interpreter":
        from co_lambda_examples._artifacts import _string_term
    ...
    from co_lambda._defun_runtime import Thunk
    from co_lambda._defunctionalize import reify
...
def _measure_defun(approach: str, input_key: str) -> "tuple[float, float, int, str]":
    ...
    from co_lambda._defun_runtime import Thunk, _BOTTOM
...
def _spawn(approach: str, app: str, input_key: str) -> Cell:
    completed = subprocess.run(
        [sys.executable, "-m", "co_lambda_examples._benchmark", "--measure", approach, app, input_key],
...
def _bootstrap_input_available() -> bool:
    """Whether the heavy ``input_quote_defun`` artifact exists for the running Python (3.11-only)."""
    import importlib.util

    from co_lambda_examples._artifacts import module_dotted_name

    return importlib.util.find_spec(module_dotted_name("input_quote_defun")) is not None


def comparison_rows(*, include_heavy: bool) -> "list[ComparisonRow]":
    """Run every cell both ways (each in its own subprocess) and return the comparison rows."""
    rows: "list[ComparisonRow]" = []
    bootstrap_ready = _bootstrap_input_available()
    for name, app, input_key, heavy in _CELLS:
        if heavy and (not include_heavy or not bootstrap_ready):
            continue
...
def benchmark_fragment(*, include_heavy: bool) -> str:
    """The committed LaTeX fragment: the interpreted-vs-compiled comparison tabular and a header."""
    rows = comparison_rows(include_heavy=include_heavy)
    parts = [
...
def main() -> None:
    ...
    matrix, print it, and write the LaTeX fragment. The heavy bootstrap runs only under
    ``FOL_REGEN_DEFUN=1``."""
    if len(sys.argv) >= 5 and sys.argv[1] == "--measure":
        _worker(sys.argv[2], sys.argv[3], sys.argv[4])
        return

    include_heavy = os.environ.get("FOL_REGEN_DEFUN") == "1"
    rows = comparison_rows(include_heavy=include_heavy)
    print(
    ...
    _OUTPUT.write_text(benchmark_fragment(include_heavy=include_heavy))
